...
const continentCoordinates: Record<string, { 
  ...
  'North America': { 
    lat: 45.0, lng: -100.0, // Geographic center of North America
  ...
  'South America': { 
    lat: -15.0, lng: -60.0, // Geographic center of South America
  ...
  'Europe': { 
    lat: 54.0, lng: 15.0, // Geographic center of Europe
  ...
  'Africa': { 
    lat: 0.0, lng: 20.0, // Geographic center of Africa
  ...
  'Asia': { 
    lat: 35.0, lng: 100.0, // Geographic center of Asia
  ...
  'Oceania': { 
    lat: -25.0, lng: 140.0, // Geographic center of Australia/Oceania
...
const Globe = ({ onLocationClick }: { onLocationClick: (location: CargoLocationData) => void }) => {
  ...
  const getPosition = (lat: number, lng: number, radius: number) => {
    // Precise spherical to Cartesian conversion for exact Earth texture mapping
    const latRad = (lat * Math.PI) / 180;
    const lngRad = ((lng + 180) * Math.PI) / 180; // Adjust longitude offset for texture alignment
    
    // Convert to Cartesian coordinates with precise texture mapping
  ...
  return (
    <group ref={groupRef}>
      ...
      {cargoLocations.map((location) => (
        <Sphere
          key={location.id}
          args={[0.12, 16, 16]} // Slightly larger for continents
          position={getPosition(location.lat, location.lng, globeRadius + 0.02)}
          onClick={(e) => {
            e.stopPropagation();
      ...
      {/* Enhanced bright and bluish lighting */}
      <ambientLight intensity={0.8} color="#B3E5FC" />
      <directionalLight 
        position={[5, 3, 5]} 
        intensity={2.0} 
        color="#E1F5FE"
      ...
      {selectedLocation && (
        <div className="absolute top-4 right-4 bg-white p-4 rounded-lg shadow-lg max-w-sm">
          ...
          <div className="text-sm">
            <div className="mb-2">
              <strong>Common Cargo Types:</strong>
